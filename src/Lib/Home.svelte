<script>
    ...
    let q = query(collection(db, "notices"), where("to", "==", $dbUser.email));
    let notifs = [];
    ...
    let taskQ = query(
        collection(db, "tasks"),
        where($dbUser.role == "o" ? "owner" : "toMail", "==", $dbUser.email)
    );
...
<div>
    ...
    <div class="taskList">
        {#each Tasks as Task}
            <div class="taskItem">
                <h3 class="prog">
                    {Math.floor(Math.random() * 90)} % Progress
                </h3>
                <h1>{Task.task}</h1>
                <h3 class="date">
